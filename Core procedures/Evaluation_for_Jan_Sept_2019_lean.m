...
function [data, BM] = Evaluation_for_Jan_Sept_2019(data, Variables)
...
% Early polyps male
OutputValues = CalculateAgreement(EarlyPolyps{1}, Variables.Benchmarks, 'EarlyPolyp', 'Male_y', 'Male_perc','Polyp'); 
BM.OutputValues.EarlyAdenoma_Male = OutputValues;

% Early polyps female
OutputValues = CalculateAgreement(EarlyPolyps{2}, Variables.Benchmarks, 'EarlyPolyp', 'Female_y', 'Female_perc', 'Polyp'); 
BM.OutputValues.EarlyAdenoma_Female = OutputValues;

% advanced polyps male
OutputValues = CalculateAgreement(AdvPolyps{1}, Variables.Benchmarks, 'AdvPolyp', 'Male_y', 'Male_perc', 'Polyp'); 
BM.OutputValues.AdvAdenoma_Male = OutputValues;

% advanced polyps female
OutputValues = CalculateAgreement(AdvPolyps{2}, Variables.Benchmarks, 'AdvPolyp', 'Female_y', 'Female_perc', 'Polyp'); 
...
OutputValues = CalculateAgreement(Incidence, Variables.Benchmarks, 'Cancer', 'Ov_y', 'Ov_inc', 'Cancer');  
...
OutputValues = CalculateAgreement(Incidence{1}, Variables.Benchmarks, 'Cancer', 'Male_y', 'Male_inc', 'Cancer');  
BM.OutputValues.Cancer_Male = OutputValues;

% female cancer incidence
OutputValues = CalculateAgreement(Incidence{2}, Variables.Benchmarks, 'Cancer', 'Female_y', 'Female_inc', 'Cancer');  
