...
DEFAULT_OPTIONS = PSOSET('SWARM_SIZE',24,... % number of particles in swarm for each variable to be optimized
...
for i = 1:OPTIONS.MAX_ITER,
    ...
    %if ~mod(i,10)
        %toSave = GBEST(i,:);
        %save('tmpN2.mat','toSave');
    %end
    
    % update the VELOCITIES
    
    VELOCITIES(:,:,i+1) = OPTIONS.ConstrictionFactor.*(VELOCITIES(:,:,i) + OPTIONS.COGNITIVE_ACC.*rand(OPTIONS.SWARM_SIZE,NDIM).*(PBEST(:,:,i)-SWARM(:,:,i)) + OPTIONS.SOCIAL_ACC.*rand(OPTIONS.SWARM_SIZE,NDIM).*(repmat(GBEST(i,:),[OPTIONS.SWARM_SIZE 1 1])-SWARM(:,:,i)));
